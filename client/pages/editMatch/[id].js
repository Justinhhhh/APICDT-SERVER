...
function EditNews({ match, schoolS }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}matches/${data.id}`, {
            method: 'PUT',
            body: JSON.stringify({
                data: {
                    teamA: teamA,
...
export async function getServerSideProps(context) {
    ...
    const schoolResponse = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}schools`)
